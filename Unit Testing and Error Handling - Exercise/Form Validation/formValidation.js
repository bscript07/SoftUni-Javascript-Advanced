function solve() {
  ...
  company.addEventListener("change", () => {
    document.getElementById("companyInfo").style.display = company.checked
      ? "block"
      : "none";
  });

  document.getElementById("submit").addEventListener("click", validateFields);
  ...
  function isCorrectPassword(password) {
    ...
    return (
      ...
      password.localeCompare(confirmPassword.value) === 0
    );
  }
  ...
  function validateFields(ev) {
    ...
    if (
      !isCorrectPassword(password.value) ||
    ...
    document.getElementById("valid").style.display = errFields.length
      ? "none"
      : "block";
  }
}
